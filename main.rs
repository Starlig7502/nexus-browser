...
</div><div id="ca"><div id="st"><h1 data-i18n="title">NEXUS</h1><div class="s" data-i18n="sub">ELITE RUST // AI INTEGRATED</div><input type="text" id="ss" placeholder="Query..." onkeydown="if(event.key==='Enter'){v('u',this.value);sr('nav',this.value)}"></div></div></div>
<div id="ap">
...
<script>
window.CL='en';
const L={en:{search:"Search...",ssearch:"Query...",title:"NEXUS",sub:"ELITE RUST // AI INTEGRATED",priv:"🛡 SHIELD"},vi:{search:"Tìm...",ssearch:"Truy vấn...",title:"NEXUS",sub:"RUST CAO CẤP // AI TÍCH HỢP",priv:"🛡 BẢO MẬT"}};
        ...
        if let Ok(p) = serde_json::from_str::<serde_json::Value>(req.body()) {
            ...
            match a {
                ...
                "ai" => if let Some(prompt) = d.as_str() {
                    ...
                    tokio::spawn(async move {
                        if let Ok(mut g) = ist_c.write() { g.push_ai("user".into(), p_str.clone()); }
                        tokio::time::sleep(std::time::Duration::from_millis(400)).await;
                        let mem_size = ist_c.read().map(|g| g.ai_mem.len()).unwrap_or(0);
                        let api_k = ist_c.read().map(|g| g.api_key.clone()).unwrap_or_default();
                        // FIX: Safe UTF-8 boundary slicing to prevent panics
                        ...
                        if let Ok(mut g) = ist_c.write() { g.push_ai("ai".into(), reply.clone()); }
